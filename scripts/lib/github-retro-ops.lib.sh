...
forge_create_issue() {
  ...
  if [[ -s "${err_file}" ]]; then
    echo "::warning::gh issue create stderr for ${repo}: $(_gha_sanitize "$(cat "${err_file}")")" >&2
  fi
  rm -f "${err_file}"
  printf '%s\n' "${url}"
}
...
forge_verify_issue_label() {
  ...
  if [[ ${rc} -ne 0 ]]; then
    echo "::warning::unable to verify ${label} label on $(_gha_sanitize "${issue_url}"): $(_gha_sanitize "${labels_out}")"
    return 0
  fi
